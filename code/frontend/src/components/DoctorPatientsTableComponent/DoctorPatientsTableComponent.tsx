...
const DoctorPatientsTableComponent = () => {
  const [search, setSearch] = useState("");

  const { data, isFetching } = useGetPatientListForDoctorQuery({
    params: {
      search,
    },
  });
              ...
              (data as [])?.map((patient: any, i) => (
                <tr key={i}>
                  ...
                  <td className="flex gap-2">
                    <Link
                      to={{
                        pathname: `/${Role.Doctor}/patients/${patient.id}`,
...
export default DoctorPatientsTableComponent;
